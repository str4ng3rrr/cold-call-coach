...
import type { TestScript, CallRecord, CallbackRecord } from '../types/scriptTesting'
...
export function useTestScripts() {
  ...
  const updateScript = useCallback((id: string, updates: Partial<Pick<TestScript, 'name' | 'archived'>>) => {
    setScripts(prev => {
